...
routes.post('/', middleware.auth, (req, res, next) =>
  Promise.resolve(_.pick(req, ['user', 'body.employer']))
    .then(({user, employerParams}) => sessionFilters.isAllowed(user, employerParams, employerParams))
    .then(([employerParams]) => new Employer(employerParams))
    .then((employer) => employer.save())
