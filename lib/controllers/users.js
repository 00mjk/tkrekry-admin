  ...
  if ( req.user.role === 'admin' ) {
    ...
    User.find({}).populate('employers').exec(function ( err, users ) {
      ...
      conf.rows = _.map( users, function ( user ) {
        var employers = _.compact(_.pluck(user.employers, 'name')),
            domain = _.compact(_.pluck(user.employers, 'domain'));

        var domainName = (_.isEmpty(domain) ? 'Tyhjä' : _.pluck(domain, 'name')),
            employerName = (_.isEmpty(domain) ? 'Tyhjä' : employers);

        return [user.first_name, user.last_name, user.phone,
                user.email, user.fallback_email, user.last_login,
                user.login_count, user.role, employerName, domainName];
      } );
    ...
    if ( req.user.role === 'admin' ||  user.authenticate( currentPassword ) ) {
      ...
      if ( req.user.role === 'admin' ) {
        if ( _.isArray( employers ) && employers.length > 0 ) {
          var updateEmployers = _.pluck( employers, '_id' );
