  ...
  if ( req.user.role === 'admin' ) {
    ...
    User.find( {}).populate('employers').exec(function ( err, users ) {
      ...
      conf.rows = _.map( users, function ( user ) {
        return [user.first_name, user.last_name, user.phone, user.email, user.fallback_email, user.last_login, user.login_count, user.role ].concat(_.pluck(user.employers, 'name'));
      } );
