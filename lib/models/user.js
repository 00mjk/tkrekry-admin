...
UserSchema.path('email').validate(function (value, respond) {
  var self = this;
  this.constructor.findOne({ email: value }).then(function (user) {
    if (user) {
      if (String(self._id) === String(user._id)) {
        return respond(true);
      } else {
...
UserSchema.statics = {
  removeFromEmployer: function (employerId) {
    return this.update({
      employers: {
        $in: [employerId]
      }
  ...
  addToEmployer: function (userIds, employerId) {
    return this.update({
      _id: {
        $in: userIds
      }
